#
# Author: Chou, Ju-chieh and Yeh, Cheng-chieh
#
...
class ContentEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("content encoder: size before conv blocks: ", out.size())
        # convolution blocks
        for l in range(self.n_conv_blocks):
            y = pad_layer(out, self.first_conv_layers[l])
            y = self.norm_layer(y)
            ...
            if self.subsample[l] > 1:
                out = F.avg_pool1d(out, kernel_size=self.subsample[l], ceil_mode=True)
            out = y + out
        #print("content encoder: size after conv blocks: ", out.size())
...
class AE(nn.Module):
    ...
    def inference(self, x, x_cond):
        ...
        #print("speaker embedding size", emb.size())
        #print("content embedding size", mu.size())
